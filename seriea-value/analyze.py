...
def build_payload(fixtures, feats, odds):
    ...
    # bonus gentile ai mercati che hanno reso di piu' nel backtest (se attivo)
    mbias = {}
    if getattr(config, "PREFER_PROVEN_MARKETS", True):
        try:
            ms = json.load(open(os.path.join(DATA_DIR, "market_stats.json")))
            w = getattr(config, "PREFER_WEIGHT", 0.10)
            mbias = {k: w * max(0.0, v["rate"] - 0.5)
                     for k, v in ms.items() if v.get("n", 0) >= 20}
        except Exception:
            mbias = {}
...
def main():
    ...
    from render import render, render_howto, gate
    site_pw = os.environ.get("SITE_PASSWORD", "").strip()
    index_html = render(payload)
    howto_html = render_howto(payload)
    if site_pw:
        index_html = gate(index_html, site_pw)
        howto_html = gate(howto_html, site_pw)
        print("Sito protetto da password (gate attivo).")
    else:
        print("Nessuna password impostata: sito in chiaro (imposta SITE_PASSWORD).")
    open(os.path.join(SITE_DIR, "index.html"), "w").write(index_html)
    open(os.path.join(SITE_DIR, "come-funziona.html"), "w").write(howto_html)
